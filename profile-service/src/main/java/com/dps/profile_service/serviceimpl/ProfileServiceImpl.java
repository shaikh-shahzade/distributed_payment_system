...
public class ProfileServiceImpl implements ProfileService {
	...
	public ProfileResponse getProfile(String userId) {
		...
		SellerProfile profile = profileRepository.findById(userId)
	...
	public ProfileResponse createProfile(String userId, CreateProfileRequest request) {
		...
		SellerProfile profile = SellerProfile.builder().userId(userId).businessName(request.getBusinessName())
				.bankAccount(request.getBankAccount()).bankName(request.getBankName()).ifscCode(request.getIfscCode())
				.contactPhone(request.getContactPhone()).preferredCurrency(request.getPreferredCurrency())
				.kycStatus(SellerProfile.KycStatus.PENDING).build();
	...
	public BankAccountResponse getBankAccount(String sellerId) {
		...
		SellerProfile profile = profileRepository.findById(sellerId)
				.orElseThrow(() -> new ProfileNotFoundException("Profile not found for sellerId: " + sellerId));
		return mapToBankAccountResponse(profile);
	}

	@Override
	@CacheEvict(value = "profile", key = "#sellerId")
	public void updateKycStatus(String sellerId, SellerProfile.KycStatus status) {
		SellerProfile profile = profileRepository.findById(sellerId)
				.orElseThrow(() -> new ProfileNotFoundException("Profile not found for sellerId: " + sellerId));
	...
	private ProfileResponse mapToProfileResponse(SellerProfile p) {
		return ProfileResponse.builder().userId(p.getUserId()).businessName(p.getBusinessName())
				.bankAccount(p.getBankAccount()).bankName(p.getBankName()).ifscCode(p.getIfscCode())
				.kycStatus(p.getKycStatus().name()).preferredCurrency(p.getPreferredCurrency())
				.contactPhone(p.getContactPhone()).createdAt(p.getCreatedAt()).updatedAt(p.getUpdatedAt()).build();
	}

	private BankAccountResponse mapToBankAccountResponse(SellerProfile p) {
		return BankAccountResponse.builder().userId(p.getUserId()).bankAccount(p.getBankAccount())
				.bankName(p.getBankName()).ifscCode(p.getIfscCode()).preferredCurrency(p.getPreferredCurrency())
				.build();
	}
}
